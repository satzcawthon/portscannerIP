...
class PortScanner:
    ...
    def _scan(self, port):
        ...
        self._printer("Porta de teste: " + str(port))
    ...
    def run(self):
        print(RED + """
        --~~sSatZs~~ ->	https://github.com/satzcawthon
        """ + END)

        print(GREEN + "[+] Target: " + END + self.target)
        print(GREEN + "[+] Threads: " + END + str(self.thread))
        print(GREEN + "[+] Portas: " + END + f"{self.ports.start}-{self.ports.stop-1}")
        print(BOLD + "\n[+] Iniciar o Scan\n" + END)
        print("PORTA          SERVIÇO")
        print("----          -------")
...
def main():
    ...
    target = input("Enderesso IP ou Host: ")
    thread = int(input("threads para usar (default é 5): ") or "5")
    port_range = input("Insira o intervalo de portas a serem verificadas (default é 1-65535): ") or "1-65535"
